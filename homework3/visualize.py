...
def draw_camera(ax, R, pos, color):
    ...
    rightTriangle = np.concatenate(
        [pos, rightTopCorner, rightBotCorner], axis=0)
...
def visualize(pts, R1, T1, R2, T2, save_name):
    ...
    angle = math.degrees(
        math.acos(np.clip(np.dot(cameraPoseVector1, cameraPoseVector2), -1, 1)))
    ...
    for r in range(1, 9):
        for c in range(1, 4):
            fourCorner = np.concatenate(
                (pts[r*4+c-5, :], pts[r*4+c-4, :], pts[r*4+c, :], pts[r*4+c-1, :]))
